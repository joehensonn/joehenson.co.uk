  ...
  render() {
    ...
    return (
      <div className="flex mb-4 mt-3">
          <div className="flex w-full lg:w-1/2 md:w-1/2 sm:w-3/4 xs:w-full bg-gray-300 rounded-lg p-5">
              ...
              <div className="w-full md:ml-3 lg:w-3/4 md:w-3/4 sm:w-3/4 xs:w-full">
                  ...
                  <div className={loading ? "loading rounded-lg" : "rounded-lg"}>
                    <p className="font-black">{loading ? 'Loading...' : fetchedData.song.artist['#text']}<span className="float-right px-3" aria-label={nowPlaying ? "What I'm listening to right now" : "What I was last listening to"} data-balloon-pos="right"><FontAwesomeIcon icon={nowPlaying ? 'volume-up' : 'volume-mute'} /></span></p>
                  </div>
              </div>
          </div>
      </div>
